...
### Rule 7: 기능 구현 시 브랜치 생성 및 PR 워크플로우 준수

작업을 시작하거나 구현 계획(Implementation Plan)을 수립하기 전에 반드시 `CONTRIBUTING.md`의 브랜칭 전략을 확인하라. 에이전트는 다음 워크플로우를 강제로 따라야 한다:
1. **기능 브랜치 생성**: `git checkout -b` 명령어로 새 브랜치를 생성하고 작업을 진행한다. `main` 브랜치에 직접 커밋하거나 로컬에서 직접 병합(Merge)하는 것은 절대 금지된다.
2. **PR(Pull Request) 생성**: 작업이 완료되면 원격 저장소에 브랜치를 푸시하고 사용자에게 PR 생성을 안내하거나, 권한이 있다면 직접 PR을 생성한다.
3. **검증 및 병합**: PR을 통해 CI 상태 검증 및 리뷰가 완료된 이후에만 `main` 브랜치에 반영할 수 있다.
