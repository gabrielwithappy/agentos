...
인터뷰 완료 후 reference mission 문서로 `docs/exec-plans/archive/reference/intent/intent-YYYYMMDD-<slug>.md`에 저장한다.
...
Intent Sheet 저장 후:
```
Intent Sheet가 `docs/exec-plans/archive/reference/intent/<filename>.md`에 저장되었습니다.
이를 기반으로 writing-plans 스킬로 구체적인 실행 계획을 `docs/exec-plans/active/<filename>.md`에 작성할까요?
```
