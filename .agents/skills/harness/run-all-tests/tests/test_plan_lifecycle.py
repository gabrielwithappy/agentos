...
def test_refresh_generates_plan_json_and_board(tmp_path):
    ...
    exec_plans = tmp_path / ".agentos" / "project" / "exec-plans"
    ...
    assert [item["path"] for item in plan["active_plans"]] == [".agentos/project/exec-plans/active/2026-04-10-active.md"]
    assert [item["path"] for item in plan["reference_plans"]] == [".agentos/project/exec-plans/archive/reference/2026-04-10-reference.md"]
    assert [item["path"] for item in plan["archived_plans"]] == [".agentos/project/exec-plans/archive/2026-04-10-archived.md"]
    ...
    assert "[Active Plan](.agentos/project/exec-plans/active/2026-04-10-active.md)" in readme
    assert "[Reference Plan](.agentos/project/exec-plans/archive/reference/2026-04-10-reference.md)" in readme
    assert "[Archived Plan](.agentos/project/exec-plans/archive/2026-04-10-archived.md)" in readme
...
def test_refresh_projects_user_outcome_and_progress(tmp_path):
    ...
    traces.mkdir(parents=True)
    (traces / "loop-state.md").write_text('plan_path: ""\n', encoding="utf-8")
    active = tmp_path / ".agentos" / "project" / "exec-plans" / "active"
    active.mkdir(parents=True)
    ...
    assert "[Legacy Plan](.agentos/project/exec-plans/active/2026-04-10-legacy.md) | reviewed" in readme
...
def test_refresh_uses_loop_state_for_current_plan(tmp_path):
    ...
    traces = tmp_path / ".agents" / "traces" / "harness"
    traces.mkdir(parents=True)
    (traces / "loop-state.md").write_text('plan_path: ".agentos/project/exec-plans/active/2026-04-10-active.md"\n', encoding="utf-8")
    exec_plans = tmp_path / ".agentos" / "project" / "exec-plans"
    ...
    plan = json.loads((tmp_path / ".agents" / "mission" / "plan.json").read_text(encoding="utf-8"))
    assert plan["current_plan"] == ".agentos/project/exec-plans/active/2026-04-10-active.md"
    assert plan["active_plans"][0]["status"] == "구현 계획 (실행 대기)"
    assert "current_plan_warning" not in plan

    readme = (tmp_path / ".agentos" / "project" / "exec-plans" / "README.md").read_text(encoding="utf-8")
    assert "- 현재 실행 중: `.agentos/project/exec-plans/active/2026-04-10-active.md`" in readme
...
def test_refresh_keeps_review_pending_active_plan_in_active_registry(tmp_path):
    ...
    plan = json.loads((tmp_path / ".agents" / "mission" / "plan.json").read_text(encoding="utf-8"))
    assert [item["path"] for item in plan["active_plans"]] == [".agentos/project/exec-plans/active/2026-04-10-review-pending.md"]

    readme = (tmp_path / ".agentos" / "project" / "exec-plans" / "README.md").read_text(encoding="utf-8")
    assert "[Review Pending Plan](.agentos/project/exec-plans/active/2026-04-10-review-pending.md)" in readme
    assert "- 현재 실행 중인 계획 없음" not in readme
    assert "- older active plans omitted=0" in readme
...
def test_refresh_surfaces_current_plan_path_mismatch_warning(tmp_path):
    ...
    traces = tmp_path / ".agents" / "traces" / "harness"
    traces.mkdir(parents=True)
    (traces / "loop-state.md").write_text('plan_path: ".agentos/project/exec-plans/active/2026-04-10-missing.md"\n', encoding="utf-8")
    exec_plans = tmp_path / ".agentos" / "project" / "exec-plans"
    active_dir = exec_plans / "active"
    active_dir.mkdir(parents=True)
    ...
    assert plan["current_plan_warning"] == {
        ...
        "plan_path": ".agentos/project/exec-plans/active/2026-04-10-missing.md",
    }

    readme = (tmp_path / ".agentos" / "project" / "exec-plans" / "README.md").read_text(encoding="utf-8")
    assert "- 현재 실행 중인 계획 없음" not in readme
    assert "plan_path mismatch" in readme
    assert ".agentos/project/exec-plans/active/2026-04-10-missing.md" in readme
    assert "- older active plans omitted=0" in readme
...
def test_set_status_rejects_in_progress_status(tmp_path):
    ...
    result = run_cli(tmp_path, "set-status", ".agentos/project/exec-plans/active/2026-04-10-active.md", "진행 중", check=False)
...
def test_archive_moves_plan_and_reclassifies_board(tmp_path):
    ...
    run_cli(tmp_path, "archive", ".agentos/project/exec-plans/active/2026-04-10-active.md", "--status", "완료")
    ...
    plan = json.loads((tmp_path / ".agents" / "mission" / "plan.json").read_text(encoding="utf-8"))
    assert plan["current_plan"] is None
    assert [item["path"] for item in plan["archived_plans"]] == [".agentos/project/exec-plans/archive/2026-04-10-active.md"]

    readme = (tmp_path / ".agentos" / "project" / "exec-plans" / "README.md").read_text(encoding="utf-8")
    assert "- 현재 실행 중인 계획 없음" in readme
    assert "[Active Plan](.agentos/project/exec-plans/archive/2026-04-10-active.md)" in readme
    assert "- older archived plans omitted=0" in readme
...
@pytest.mark.parametrize(
        ...
        [
            (("set-status", ".agentos/project/exec-plans/active/2026-04-10-a.md", "엉뚱한 상태"), "invalid status"),
            (("set-status", ".agentos/project/exec-plans/active/missing.md", "구현 계획 (실행 대기)"), "plan not found"),
            (("archive", ".agentos/project/exec-plans/archive/2026-04-10-archived.md", "--status", "완료"), "already archived"),
            (("archive", ".agentos/project/exec-plans/active/2026-04-10-a.md", "--status", "보관됨"), "archive only supports"),
        ],
)
def test_lifecycle_commands_reject_invalid_inputs_without_artifact_changes(tmp_path, command, expected_error):
    ...
    before_readme = (tmp_path / ".agentos" / "project" / "exec-plans" / "README.md").read_text(encoding="utf-8")
    ...
    assert (tmp_path / ".agentos" / "project" / "exec-plans" / "README.md").read_text(encoding="utf-8") == before_readme
...
def test_archive_rejects_destination_collision_without_artifact_changes(tmp_path):
    ...
    result = run_cli(tmp_path, "archive", ".agentos/project/exec-plans/active/2026-04-10-a.md", "--status", "완료", check=False)
...
def test_refresh_keeps_reference_plans_separate_from_archived(tmp_path):
    ...
    plan = json.loads((tmp_path / ".agents" / "mission" / "plan.json").read_text(encoding="utf-8"))
    assert [item["path"] for item in plan["reference_plans"]] == [".agentos/project/exec-plans/archive/reference/2026-04-10-b.md"]
    ...
    assert "## Reference Docs" in readme
    assert "- older reference docs omitted=0" in readme
    assert "[Reference Plan](.agentos/project/exec-plans/archive/reference/2026-04-10-b.md)" in readme
...
def test_refresh_generates_archived_summary_for_completed_and_parked_only(tmp_path):
    ...
    assert [item["path"] for item in plan["archived_plans"]] == [
        ".agentos/project/exec-plans/archive/2026-04-10-complete.md",
        ".agentos/project/exec-plans/archive/2026-04-10-parked.md",
    ]
...
def test_refresh_supports_nested_reference_taxonomy(tmp_path):
    ...
    assert [item["path"] for item in plan["reference_plans"]] == [
        ".agentos/project/exec-plans/archive/reference/intent/intent-20260414-sample.md"
    ]

    readme = (tmp_path / ".agentos" / "project" / "exec-plans" / "README.md").read_text(encoding="utf-8")
    assert "- older reference docs omitted=0" in readme
    assert "[Intent Sheet](.agentos/project/exec-plans/archive/reference/intent/intent-20260414-sample.md)" in readme
