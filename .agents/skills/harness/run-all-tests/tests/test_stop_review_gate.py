...
def workspace(tmp_path):
    ...
    # Symlink the hook so we can run it
    ...
    # Needs to find review_artifacts.py in the real path or we can mock SCRIPT_DIR
    # but the script resolves __file__ to find it. Since it's a symlink, resolve() will point to the real path.
    
    # create active plan dir
...
def test_invalid_review_warns_and_continues(workspace):
    ...
    assert warn["plan_path"] == ".agentos/project/exec-plans/active/plan.md"
    assert "invalid-review-evidence" in warn["detail"] or "missing=" in warn["detail"]

def test_valid_review_has_no_warning(workspace):
    # We can mock review_artifacts check_plan or just create valid artifacts.
    # To create valid artifacts, we need to use review_artifacts.py directly.
    import sys
...
def test_malformed_artifact_is_redacted_invalid(workspace):
    ...
    # write malformed JSON to principle-auditor with secret content
...
def test_warning_redacts_artifact_content(workspace):
    # already tested mostly in malformed_artifact_is_redacted_invalid, 
    # but we can ensure valid artifacts aren't exposed either.
    pass
...
def test_loop_lock_blocks(workspace):
    ...
    assert "execution-locked" in res.get("reason", "") or "loop is still execution-locked" in res.get("reason", "")
...
def test_unverified_completion_blocks(workspace):
    ...
    (workspace / "dirty").write_text("dirty2") # now dirty
    
    payload = {"cwd": str(workspace), "last_assistant_message": "I finished it."}
    res = run_hook(workspace, payload)
    assert res.get("decision") == "block"
    assert "verification status with evidence" in res.get("reason", "")
