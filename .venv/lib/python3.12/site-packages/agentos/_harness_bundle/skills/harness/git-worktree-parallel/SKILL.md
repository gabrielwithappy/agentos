...
- 브랜치는 목적과 종류가 드러나게 짓는다
  ...
  - 예: `spike/<topic>`
  - 예: `hotfix/<topic>`
- 같은 이름이 이미 있으면 재사용 여부를 먼저 확인하고 임의로 덮어쓰지 않는다. 충돌 시 `git worktree list`로 확인하고 새 `--branch`/`--path`를 선택하거나 기존 worktree를 명시적으로 재사용한다.
...
`--path`를 생략하면 `<repo>/.agentos/worktrees/<branch-slug>`가 사용된다.

```bash
# 기본 AgentOS worktree 위치 사용
bash .agents/skills/harness/git-worktree-parallel/scripts/create-worktree.sh \
  --branch feature/example --base HEAD

# 명시 위치 사용 (기존 호환)
bash .agents/skills/harness/git-worktree-parallel/scripts/create-worktree.sh \
  --branch feature/example --path /explicit/path --base HEAD
```
