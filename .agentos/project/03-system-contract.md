...
- credential boundary: VS Code Webview must not own raw credentials; Python CLI may own a provider-independent auth store foundation for approved local metadata/credential records, while the current `codex` runtime path remains an external CLI compatibility path. Future native OAuth/transport requires a separate reviewed plan.
