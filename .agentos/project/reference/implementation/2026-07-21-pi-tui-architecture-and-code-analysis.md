...
1. **분기(Branch) 탐색기 트리 뷰 (`/tree`)**
   - 대화 내역의 분기를 그래픽적인 ASCII 트리로 시각화.
   - Textual의 Tree 위젯을 응용해야 하지만, AgentOS의 대화 상태 관리(Session) 구조가 분기 모델을 완벽히 지원하도록 변경된 후에만 가능.
2. **도구별 커스텀 렌더러 (Custom Tool Output Renderers)**
   - 도구 실행 결과(예: diff, 파일 목록, 검색 결과 등)를 단순 텍스트가 아닌 풍부한 UI 컴포넌트로 렌더링.
   - AgentOS에 도구별 UI 확장을 허용하는 Plugin 아키텍처 도입이 선행되어야 함.
3. **고도화된 Diff 렌더러**
