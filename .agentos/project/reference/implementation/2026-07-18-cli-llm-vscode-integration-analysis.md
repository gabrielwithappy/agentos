...
- 2026-07-23 기준 `agentos/llm/providers/codex_cli.py`는 Codex CLI stdout을 process 완료 후 재생하지 않고 line-by-line으로 읽어 normalized event를 더 이르게 방출한다.
- 2026-07-24 addendum: AgentOS가 native Codex auth/transport를 직접 소유하도록 승인되었다. `agentos/llm/auth/openai_codex.py`가 documented OpenAI Codex account-login flow(browser callback 우선, device-code fallback을 같은 login 흐름 안에서 제공)로 login/refresh/logout/status를 소유하고, `agentos/llm/transports/openai_codex_responses.py`가 WebSocket 우선/SSE fallback native streaming transport를 소유한다. `agentos/llm/providers/codex_cli.py`(external CLI compatibility path)는 native provider가 canonical이 된 이후 native 실패 시에만 선택되는 recovery-only debug/rollback path로 재분류된다.
