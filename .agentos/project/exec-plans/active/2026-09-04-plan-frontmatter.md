...
|---|---|
| 전체 상태 | 리뷰 대기 |
| 완료됨 | 기존 parser의 frontmatter 지원과 lifecycle/review의 legacy blockquote 의존성 조사 |
| 현재 위치 | Gate 2 독립 리뷰 전 |
| 다음 단계 | 리뷰 PASS와 protected approval 후 template·parser·검증 경로 구현 |
...
- [ ] **Step 2.1: template을 frontmatter canonical 예시로 바꾼다.**
...
- [ ] **Step 2.2: writing-plans guidance에 migration/fallback 규칙을 명시한다.**
...
- [ ] **Step 3.1: 공통 scalar frontmatter parsing과 metadata upsert를 안전하게 정리한다.**
...
- [ ] **Step 4.1: frontmatter/legacy/혼합 경계 회귀를 추가한다.**
...
Gate 2 reviewer artifact와 protected architect approval은 구현 Task가 아니라 lifecycle 단계에서 확인한다. `reviewed: true`는 세 reviewer의 PASS와 required scope approval이 현재 semantic plan에 유효할 때만 설정한다.
