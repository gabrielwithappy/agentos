...
| REQ-KNOWLEDGE-001 | standalone knowledge-curator 기반 장기지식 관리 | must | OKF v0.2 Markdown bundle, `init`/`validate`/`inspect`/`reorganize`/`backup`/`sync` 흐름과 local Git safety boundary가 standalone curator skill에서 검증됨. Knowledge 문서는 evidence이며 root docs, active plan, Gate 2, protected-path rules를 override하지 않음 | `catalog/skills/knowledge-curator/SKILL.md`, `docs/knowledge/README.md`, `.agentos/project/exec-plans/active/2026-08-23-remove-agentos-knowledge-runtime.md` | `pytest tests/test_knowledge_skill.py tests/test_knowledge_curator_evals.py -q`; standalone CLI help; OKF validation suite | 현재 |
