...
장기지식이나 이전 조사 결과가 필요하면 `docs/knowledge/README.md`에서 pointer-only 안내를 확인한 뒤 canonical repository인 `https://github.com/gabrielwithappy/knowledge-agent`를 사용한다. 이 결과는 재사용 가능한 evidence이며 root project documents, active plan, Gate 2 reviewer authority, protected-path rules를 override하지 않는다.
...
| `docs/knowledge/README.md` and `docs/knowledge/index.md` | `02-product-scope-and-requirements.md`, `03-system-contract.md`, `04-safety-risk-verification.md`, and `06-decisions-change-log.md` | 현재 | `knowledge-agent` canonical repository의 pointer-only 진입점을 제공하고 이후 계획에서 재사용 가능한 조사 근거를 찾게 함. | Refresh when `knowledge-agent` repository location, knowledge-curator validation flow, metadata contract, or authority boundary changes. |
...
| `reference/implementation/2026-07-21-pi-tui-architecture-and-code-analysis.md` | `03-system-contract.md` | 현재 | AgentOS TUI와 pi TUI 간의 기능 격차를 분석하고 향후 이식 가능한 기능들의 아키텍처 구조 보존. | Refresh when pi TUI's additional analysis is performed or AgentOS TUI is significantly refactored. |
| `reference/implementation/2026-07-22-pi-hermes-tui-feature-inventory.md` | `03-system-contract.md` | 현재 | pi TUI + hermes TUI 전체 기능 인벤토리 (Phase 3+ 대상) 및 AgentOS 이식 매핑. Phase 1·2 이후 미구현 항목 상세 분석, Phase 3 우선순위 제안, AgentOS TUI 현재 아키텍처 구조 보존. | Refresh when pi/hermes sources significantly change or AgentOS TUI completes a new Phase. |
| `reference/implementation/2026-07-24-agentos-pi-session-runtime-contract.md` | `02-product-scope-and-requirements.md`, `03-system-contract.md`, and `04-safety-risk-verification.md` | 현재 | PI의 conversation context/continuation ownership을 AgentOS Python/Textual runtime으로 이식하기 위한 target contract: message schema, continuation privacy, snapshot-rebuild/migration policy, PI mapping. | Refresh when the reviewed session-runtime plan changes, native Codex transport is implemented, or the conversation persistence contract changes. |
