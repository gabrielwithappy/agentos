...
```
.agents/
  plugins/                    ← 설치된 플러그인 보관 및 잠금 파일 관리
  skills/harness/core-engine/ ← 루프 실행 엔진 (수정 금지)
  skills/harness/brain/       ← 공유 에이전트 지능 (protocols + lessons-learned.md)
  skills/harness/             ← 하네스 기본 스킬 (수정 금지)
  agents/harness/             ← 하네스 기본 에이전트 (수정 금지)
  hooks/                      ← Codex/Claude Code native hook 동작 원칙과 SSOT
  vendors/                    ← vendor-specific guide 계층
  mission/                    ← 과업 목표 및 설계 (plan.json)
  traces/                     ← 실행 추적 및 상태 (휘발성)
```

런타임 지원 경로:
- `.claude/` — Claude 런타임 설정 및 hook 연결
- `.codex/` — Codex 런타임 설정 및 hook 연결
- Discord bridge/pool 운영 스크립트와 런타임 데이터는 AHA profile home(`$AHA_HOME/profiles/<profile>/discord/`)에서 관리한다.
- 루트 `harness-loop.sh` — 하위 호환 shim
- canonical harness loop 진입점 — `.agents/skills/harness/harness-loop.sh`

**예약 이름 — 사용자 정의 이름으로 쓰지 마라**: `harness`, `mission`, `traces`, `_version`

### .agents/ 수정 규칙

- `.agents/_version.json`의 `authorized_architects`만 수정할 수 있다
- 수정 후 반드시 `.agents/skills/harness/sync-manifest/scripts/sync-manifest.sh --update`를 실행하라
- `.agents/skills/harness/run-all-tests/tests/run_all_tests.sh`는 매니페스트와 파일 구조가 100% 일치할 때만 실행된다

### Vendor Guide 계층

- `.agents/vendors/*.md`는 vendor-specific 해석/실행 guide 계층이다.
- `.agents/hooks`는 hook SSOT이며 Codex/Claude Code native hook adapter는 여기의 공통 원칙과 스크립트를 참조한다.
- 공통 역할 정의의 SSOT는 `.agents/agents/harness/*.md`이다.
- 공통 skill 정의의 SSOT는 `.agents/skills/harness/*`이다.
- vendor guide는 공통 정의층을 복제하지 않고, 각 런타임이 이를 어떻게 읽고 해석할지 설명한다.

### 스킬 로딩 순서

1. `.agents/skills/<name>/SKILL.md` — 사용자 정의 스킬 (사용 시 "사용자 스킬 사용 중: `<name>`" 명시)
2. `.agents/skills/harness/<name>/SKILL.md` — 하네스 기본 스킬

### 에이전트 로딩 원칙

1. 공통 역할 정의는 `.agents/agents/harness/*.md`를 기준으로 읽는다.
2. vendor-specific native agent registry가 필요한 경우에만 각 런타임의 별도 등록 구조를 사용한다.
3. vendor guide는 역할 해석 규칙을 제공하지만 native agent를 자동 생성하지는 않는다.
4. `requirement-discovery`가 `Requirement Brief`를 생성한 경우, `goal-alignment-reviewer` PASS 후에만 `intent-clarification`으로 진행한다.

### 기본 스킬 목록

| 스킬 | 용도 |
|------|------|
| `requirement-discovery` | 사용자가 아직 명확히 설명하지 못한 구현 요구사항을 대화로 수렴하고 `Requirement Brief`와 개발 입력 문서 패키지(`User Stories`, `RTM`, `Implementation Guide`)로 문서화 |
| `intent-clarification` | 다단계 작업 전 의도·제약·성공 기준 인터뷰 (writing-plans 직전 게이트) |
| `writing-plans` | 다단계 작업 계획 작성 |
| `verification-before-completion` | 완료 전 증거 기반 검증 |
| `executing-plans` | 계획 단계별 실행 |
| `pm` | 요구사항 분해 및 태스크 계획 |
| `debug` | 버그 진단 및 수정 |
| `qa` | 품질 검증 및 보안 리뷰 |
| `brainstorm` | 아이디어 탐색 및 설계 |
| `sync-manifest` | 하네스 무결성 동기화 |
| `run-all-tests` | 전체 기능 및 무결성 테스트 |
| `mcp` | MCP 관리 (browse/install/list/remove) |
| `harness-loop` | 자연어 프롬프트로 하네스 루프 백그라운드 실행 |

도메인 플러그인(`backend`, `frontend`, `mobile`, `db`, `tf-infra`, `translator`, `dev-workflow`)은 프로젝트 성격에 맞는 승인된 설치 절차로 별도 설치하라.

---

## 상태 파일

- **루프 상태**: `.agents/traces/harness/loop-state.md` (`/harness-loop` 등 루프 모드에서 사용)
- **미션 로드맵(사용 경로)**: `.agents/mission/plan.json`
- **매니페스트**: `.agents/_version.json`
